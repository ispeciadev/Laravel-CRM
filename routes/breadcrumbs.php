...
// Dashboard > Deals
Breadcrumbs::for('deals', function (BreadcrumbTrail $trail) {
    $trail->parent('dashboard');
    $trail->push(trans('admin::app.layouts.deals') ?? 'Deals', route('admin.deals.index'));
});

// Dashboard > Deals > Create
Breadcrumbs::for('deals.create', function (BreadcrumbTrail $trail) {
    $trail->parent('deals');
    $trail->push('Create Deal', route('admin.deals.create'));
});

// Dashboard > Deals > Edit
Breadcrumbs::for('deals.edit', function (BreadcrumbTrail $trail, $deal) {
    $trail->parent('deals');
    $trail->push('Edit Deal', route('admin.deals.edit', $deal->id));
});

// Dashboard > Deals > View
Breadcrumbs::for('deals.view', function (BreadcrumbTrail $trail, $deal) {
    $trail->parent('deals');
    $trail->push('#'.$deal->id, route('admin.deals.view', $deal->id));
});
...
Breadcrumbs::for('settings.email_accounts', function (BreadcrumbTrail $trail) {
    $trail->parent('settings');
    $trail->push(trans('admin::app.settings.email-accounts.index.title'), route('admin.settings.email_accounts.index'));
});

// Dashboard > Email Accounts > Create Email Account
Breadcrumbs::for('settings.email_accounts.create', function (BreadcrumbTrail $trail) {
    $trail->parent('settings.email_accounts');
    $trail->push(trans('admin::app.settings.email-accounts.create.title'), route('admin.settings.email_accounts.create'));
});

// Dashboard > Email Accounts > Edit Email Account
Breadcrumbs::for('settings.email_accounts.edit', function (BreadcrumbTrail $trail, $id = null) {
    $trail->parent('settings.email_accounts');
    $trail->push(trans('admin::app.settings.email-accounts.edit.title'), route('admin.settings.email_accounts.edit', $id));
});
