<x-admin::layouts>
    ...
    <form
        method="POST"
        action="{{ route('admin.voip.trunks.update', $trunk->id) }}"
        class="mt-8"
        x-data="{
            authMethod: '{{ old('auth_method', $trunk->auth_method ?? 'username_password') }}',
            showPassword: false
        }"
    >
        @method('PUT')
                ...
                <div class="mb-4">
                    <label class="block text-sm text-gray-800 dark:text-white font-medium mb-1.5">Provider</label>
                     <select
                        ...
                        class="w-full py-2.5 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 dark:bg-gray-900 dark:border-gray-800"
                    >
                        <option value="" {{ !$trunk->voip_provider_id ? 'selected' : '' }}>Custom (No Provider)</option>
                        @foreach($providers as $provider)
                            <option value="{{ $provider->id }}" {{ old('voip_provider_id', $trunk->voip_provider_id) == $provider->id ? 'selected' : '' }}>
            ...
            <div class="p-4">
                ...
                <div class="mb-4">
                    ...
                    <input
                        type="number"
                        name="port"
                        value="{{ old('port', $trunk->port ?? 5060) }}"
                        min="1"
                        max="65535"
                        class="w-full py-2.5 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 dark:bg-gray-900 dark:border-gray-800"
                    />
                ...
                <div class="mb-4">
                    ...
                    <select
                        ...
                        class="w-full py-2.5 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 dark:bg-gray-900 dark:border-gray-800"
                    >
                        <option value="udp" {{ old('transport', $trunk->transport ?? 'udp') == 'udp' ? 'selected' : '' }}>UDP</option>
                ...
                <div class="mb-4">
                    ...
                    <select
                        ...
                        class="w-full py-2.5 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 dark:bg-gray-900 dark:border-gray-800"
                    >
                        <option value="username_password" {{ old('auth_method', $trunk->auth_method ?? 'username_password') == 'username_password' ? 'selected' : '' }}>Username/Password</option>
                        <option value="ip_auth" {{ old('auth_method', $trunk->auth_method) == 'ip_auth' ? 'selected' : '' }}>IP Authentication</option>
                    </select>
                    @error('auth_method')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div x-show="authMethod === 'username_password'" x-transition>
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            type="text"
                            name="sip_username"
                            value="{{ old('sip_username', $trunk->sip_username) }}"
                            class="w-full py-2.5 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 dark:bg-gray-900 dark:border-gray-800"
                    ...
                    <div class="mb-4">
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Leave blank to keep current password. Minimum 6 characters if changing.</p>
                        @error('sip_password')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div x-show="authMethod === 'ip_auth'" x-transition>
                    <div class="mb-4">
                        ...
                        >{{ old('allowed_ips', is_array($trunk->allowed_ips) ? implode("\n", $trunk->allowed_ips) : '') }}</textarea>
                ...
                <button type="submit" class="primary-button">Update Trunk</button>
            </div>
        </div>
    </form>
</x-admin::layouts>
