...
namespace Ispecia\Admin\DataGrids\Contact;

use Illuminate\Database\Query\Builder;
use Illuminate\Support\Facades\DB;
use Ispecia\Contact\Repositories\OrganizationRepository;
        ...
        $this->addColumn([
            ...
            'closure'    => function ($row) {
                ...
                return collect($contactNumbers)
                    ->map(function ($number) {
                        return '<a href="tel:' . $number['value'] . '" class="flex items-center gap-1 text-blue-600 hover:text-blue-800 cursor-pointer" onclick="handleCall(\'' . e($number['value']) . '\')">' .
                               '<span class="icon-call text-2xl"></span> ' . e($number['value']) . 
                               '</a>';
                    })
                    ->join(', ');
            },
