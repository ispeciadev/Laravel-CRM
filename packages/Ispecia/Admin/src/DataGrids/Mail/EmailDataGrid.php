...
namespace Ispecia\Admin\DataGrids\Mail;
...
use Ispecia\Tag\Repositories\TagRepository;
...
class EmailDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder(): Builder
    {
        $queryBuilder = DB::table('emails')
            ->select(
                ...
                'emails.opened_at',
                'emails.open_count',
                DB::raw('GROUP_CONCAT(DISTINCT '.DB::getTablePrefix().'tags.name) as tags'),
            ...
            ->groupBy('emails.id', 'emails.name', 'emails.from', 'emails.subject', 'emails.reply', 'emails.is_read', 'emails.created_at', 'emails.folders', 'emails.sent_at', 'emails.opened_at', 'emails.open_count')
            ->where('emails.folders', 'like', '%"'.request('route').'"%')
            ->whereNull('emails.parent_id');
    ...
    public function prepareColumns(): void
    {
        ...
        // Add tracking status column for sent folder
        if (request('route') == 'sent') {
            $this->addColumn([
                'index'      => 'tracking_status',
                'label'      => trans('admin::app.mail.index.datagrid.status'),
                'type'       => 'string',
                'searchable' => false,
                'filterable' => false,
                'sortable'   => false,
                'closure'    => function ($row) {
                    if ($row->opened_at) {
                        $openedAt = Carbon::parse($row->opened_at)->format('Y-m-d H:i');
                        return '<span class="label-active">Opened</span><br><small>' . $openedAt . '</small>';
                    }
                    
                    if ($row->sent_at) {
                        return '<span class="label-info">Sent</span>';
                    }
